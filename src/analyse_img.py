...
#-------------------- 3 x 3 image ----------------------------#
def get_click_array_3x3(classNbr):
    model = YOLO("yolov8m.pt")
    ...
    height = img.size[1]
    # left, upper, right, lower
    C1R1 = [0, 0, width / 3, height / 3]
    C2R1 = [width / 3, 0, width / 3 * 2, height / 3]
    C3R1 = [width / 3 * 2, 0, width / 3 * 3, height / 3]
    C1R2 = [0, height / 3, width / 3, height / 3 * 2]
    C2R2 = [width / 3, height / 3, width / 3 * 2, height / 3 * 2]
    C3R2 = [width / 3 * 2, height / 3, width / 3 * 3, height / 3 * 2]
    C1R3 = [0, height / 3 * 2, width / 3, height / 3 * 3]
    C2R3 = [width / 3, height / 3 * 2, width / 3 * 2, height / 3 * 3]
    C3R3 = [width / 3 * 2, height / 3 * 2, width / 3 * 3, height / 3 * 3]


    fullGrid = [C1R1, C2R1, C3R1,
                C1R2, C2R2, C3R2,
                C1R3, C2R3, C3R3]

    # shouldClick = should_click(boxResults, fullGrid)
    unique = make_unique_should_click(boxResults)
    boxesArray = boxClick(unique, fullGrid)
    print(boxesArray)
    return boxesArray